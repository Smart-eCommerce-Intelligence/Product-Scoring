...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description="Analyze product data from Shopify and WooCommerce, score products, and save results.")
...
print(f"DEBUG: Parsed analysis script arguments: {args}")

# --- Populate original global configuration variables using parsed arguments ---
DB_CONFIG_SHOPIFY = {
    'host': args.db_host, 'user': args.db_user, 'password': args.db_password, 'database': args.db_name_shopify
}
DB_CONFIG_WOOCOMMERCE = {
    'host': args.db_host, 'user': args.db_user, 'password': args.db_password, 'database': args.db_name_woocommerce
}
DB_CONFIG_ANALYSIS = {
    'host': args.db_host, 'user': args.db_user, 'password': args.db_password, 'database': args.db_name_analysis
}
print(f"DEBUG: DB_CONFIG_SHOPIFY set to: {DB_CONFIG_SHOPIFY}")
print(f"DEBUG: DB_CONFIG_WOOCOMMERCE set to: {DB_CONFIG_WOOCOMMERCE}")
print(f"DEBUG: DB_CONFIG_ANALYSIS set to: {DB_CONFIG_ANALYSIS}")

TOP_K_OVERALL = args.top_k_overall
FLAGSHIP_PER_STORE = args.flagship_per_store
WEIGHT_AVAILABILITY = args.weight_availability
WEIGHT_PRICE = args.weight_price
DB_BATCH_SIZE = args.db_batch_size # This will be used by save_scored_products_to_db
print(f"DEBUG: Analysis Params: TOP_K={TOP_K_OVERALL}, FLAGSHIP_K={FLAGSHIP_PER_STORE}, W_AVAIL={WEIGHT_AVAILABILITY}, W_PRICE={WEIGHT_PRICE}, BATCH_SIZE={DB_BATCH_SIZE}")
...
def calculate_attractiveness_score(df, w_availability, w_price):
    # This function will use the global WEIGHT_AVAILABILITY and WEIGHT_PRICE set by argparse
    if df.empty or not all(col in df.columns for col in ['is_available_numeric', 'price']):
        if not df.empty: df['final_score'] = 0.0 # Ensure column exists even if skipping
...
# These functions remain UNCHANGED from your original script
# create_analysis_tables uses 'conn_analysis'
# save_scored_products_to_db uses 'conn_analysis' and the 'batch_size' argument
# (which will be the global DB_BATCH_SIZE from argparse in the main script)
...
def save_scored_products_to_db(df, conn_analysis, batch_size=DB_BATCH_SIZE): # Default uses global
    ...
    for col in cols_for_db:
        if col not in df.columns:
            # Ensure the column exists, even if it's all None initially
            df[col] = None
    ...
    for col in text_cols_to_na:
        if col in df_to_save.columns:
             ...
             df_to_save[col] = 'N/A' # Should not happen if cols_for_db is used
    ...
    all_data_tuples = [tuple(x) for x in df_to_save[cols_for_db].to_numpy()] # Ensure column order
    total_saved_count = 0
...
def save_store_rankings_to_db(avg_scores_series, max_scores_series, platform_map_df, conn_analysis):
    ...
    if 'source_platform' not in rankings_df.columns: # Should be handled by else above
        rankings_df['source_platform'] = 'Unknown'
    else:
...
# --- Display functions ---
# These functions will use the global TOP_K_OVERALL and FLAGSHIP_PER_STORE
def display_top_k_products(df, k): # k will be TOP_K_OVERALL from global
    # This function remains UNCHANGED from your original script
...
def display_flagship_products_per_store(df, n_flagship): # n_flagship will be FLAGSHIP_PER_STORE from global
    # This function remains UNCHANGED from your original script
...
if __name__ == "__main__":
    print("--- Starting Product Analysis Script ---")
    # Connections are established using global DB_CONFIG_* dicts (populated by argparse)
    ...
    if conn_analysis:
        ...
        print("Critical: Cannot connect to Analysis DB. Exiting.")
        exit(1)
    ...
    if df_s_list:
        ...
        if not combined_df.empty:
            ...
            if not combined_df.empty:
                # Global variables WEIGHT_AVAILABILITY and WEIGHT_PRICE are used here
                combined_df = calculate_attractiveness_score(combined_df, WEIGHT_AVAILABILITY, WEIGHT_PRICE)

                # Global TOP_K_OVERALL and FLAGSHIP_PER_STORE are used here
                ...
                if conn_analysis:
                    # Global DB_BATCH_SIZE is used by default in save_scored_products_to_db
                    save_scored_products_to_db(combined_df, conn_analysis, DB_BATCH_SIZE) # Explicitly pass for clarity
                    ...
                    if not combined_df.empty and 'source_store_name' in combined_df.columns and 'source_platform' in combined_df.columns:
                        # Create a mapping of store name to platform
